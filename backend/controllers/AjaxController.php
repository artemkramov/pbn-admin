...
class AjaxController extends AuthController implements ViewContextInterface
{
     ...
     * @return array
     */
    public function actionLoadStockCities()
    {
        $cities = StockCity::find()->localized(Lang::getCurrent()->url)->orderBy('sort')->all();
        $response = [];
        foreach ($cities as $city) {
            $attributeData = $city->attributes;
            $attributeData['name'] = $city->title;
            $response[] = $attributeData;
        }
        return $response;
    }
     ...
     * @return array
     */
    public function actionLoadShops($id)
    {
        $items = Stock::find()->localized(Lang::getCurrent()->url)->where([
            'stock_city_id' => $id,
        ...
        foreach ($items as $item) {
             ...
             */
            $attributeData = $item->attributes;
            $attributeData['name'] = $item->getPostTitle();
            $response[] = $attributeData;
        }
        return $response;
    }
